...
REPLICA_LINE=""
if [[ -n "${PGBOUNCER_REPLICA_HOST:-}" ]]; then
  REPLICA_DBNAME="${PGBOUNCER_REPLICA_DBNAME:-${POSTGRESQL_DATABASE}}"
  REPLICA_USER="${PGBOUNCER_REPLICA_USERNAME:-${POSTGRESQL_USERNAME}}"
  REPLICA_PASS="${PGBOUNCER_REPLICA_PASSWORD:-${POSTGRESQL_PASSWORD}}"
  REPLICA_PORT="${PGBOUNCER_REPLICA_PORT:-5432}"
  REPLICA_ALIAS="${PGBOUNCER_REPLICA_ALIAS:-${PGBOUNCER_DATABASE}_ro}"
  REPLICA_LINE="${REPLICA_ALIAS} = host=${PGBOUNCER_REPLICA_HOST} port=${REPLICA_PORT} dbname=${REPLICA_DBNAME} user=${REPLICA_USER} password=${REPLICA_PASS}"
fi
